...
public class IndexedUserAdapter extends BaseAdapter {
	 ...
	 * @param context
	 */
	public IndexedUserAdapter (ListView listview, ArrayList<TVUser> users, Context context) {
	...
	private void initSectionIndexer () {
		for (int i = 0; i < users.size(); i++) {
			TVUser user = users.get(i);
			if (!indexer.containsKey(user.getNickname().substring(0, 1)))
				indexer.put(user.getNickname().substring(0, 1), i);
		}
